# -*- coding: utf-8 -*-
"""
HRV-LONGITUDINAL-ANALYZER - VERSIÓN PÚBLICA
--------------------------------------------
...
# Entorno público independiente.
# Este programa nunca utiliza Datos/, Resultados/ ni la selección activa privada.
DATOS_PUBLICOS_DIR = SCRIPT_DIR / "Datos_publicos"
RESULTADOS_PUBLICOS_DIR = SCRIPT_DIR / "Resultados_publicos"

BASE_DIR = DATOS_PUBLICOS_DIR
IMAGES_DIR = BASE_DIR
INFORMES_DIR = RESULTADOS_PUBLICOS_DIR
SHARED_IMAGES_DIR = SCRIPT_DIR / ASSETS_DIR_NAME / SHARED_IMAGES_SUBDIR

# Desactivar expresamente cualquier selección privada creada por main.py.
selected_paths_from_active_selection = None
print_active_selection = None
load_active_selection = None
...
def resolver_directorio_participantes(root_dir):
    """Devuelve exclusivamente la carpeta pública de datos."""
    datos_dir = _resolver_ruta(DATOS_PUBLICOS_DIR)
    if not datos_dir.exists() or not datos_dir.is_dir():
        raise FileNotFoundError(
            "No existe la carpeta pública de datos:\n"
            f"   {datos_dir}\n\n"
            "Ejecuta primero: py .\\modules\\distribution.py"
        )
    return datos_dir
...
def configurar_directorios_base(participant_dir):
    ...
    INFORMES_DIR = RESULTADOS_PUBLICOS_DIR / safe_simple_folder_name(nombre_sujeto_desde_carpeta(BASE_DIR))
...
def get_global_summary_excel_path():
    out_dir = RESULTADOS_PUBLICOS_DIR / "Seguimiento_global"
...
def ejecutar_analisis():
    ...
    carpetas_participantes = seleccionar_carpetas_participantes(DATOS_PUBLICOS_DIR)
...
def _count_processed_records_from_excels():
    resultados_dir = RESULTADOS_PUBLICOS_DIR
...
def main():
    while True:
        print("\n" + "=" * 60)
        print(" HRV-LONGITUDINAL-ANALYZER - VERSIÓN PÚBLICA")
        print("=" * 60)
        print("1. Analizar")
        print("0. Salir")
        ...
        print("Opción no válida. Introduce 1 o 0.")
