...
# Distribution Module (v1.5.0-a)
try:
    from modules.distribution import mostrar_preparar_distribucion
    DISTRIBUTION_IMPORT_ERROR = None
except Exception as exc:
    mostrar_preparar_distribucion = None
    DISTRIBUTION_IMPORT_ERROR = str(exc)

# Codificación de datos y actualización pública
try:
    from modules.public_data import (
        publish_public_data_to_github,
        show_public_data_status,
        sync_public_data,
    )
    PUBLIC_DATA_IMPORT_ERROR = None
except Exception as exc:
    publish_public_data_to_github = None
    show_public_data_status = None
    sync_public_data = None
    PUBLIC_DATA_IMPORT_ERROR = str(exc)
...
def configurar_directorios_base(participant_dir):
    ...
    # La carpeta del participante se añade una sola vez en build_output_paths().
    # Mantener aquí únicamente la raíz evita Resultados/Sujeto/Sujeto/.
    INFORMES_DIR = SCRIPT_DIR / RESULTADOS_DIR_NAME
...
def add_derived_columns(df):
    ...
    if "RMSSD" in df.columns:
        rmssd_num = pd.to_numeric(
            df["RMSSD"].astype(str).str.strip().str.replace(",", ".", regex=False),
            errors="coerce",
        )
    else:
        rmssd_num = pd.Series(np.nan, index=df.index)
    # LnRMSSD = logaritmo natural de RMSSD; solo se calcula para RMSSD > 0.
...
def build_report_metadata(run_id, nombre_sujeto, tipo_informe_label, fecha_informe, fecha_desde, fecha_hasta, momento, definition_mode, selected_vars, n_registros_informe, short_window=7, long_window=28):
...
def set_default_font(doc, font_name="Arial", font_size=10):
    """Configura una tipografía base segura para los documentos Word."""
...
def build_execution_summary_columns():
    cols = [
        ...
        "FECHA_DESDE", "FECHA_HASTA", "ORDEN_REGISTRO", "MODO_DEFINICION",
...
def build_execution_summary_row(run_id, fecha_informe, hora_informe, nombre_sujeto, tipo_informe_label,
                                fecha_desde, fecha_hasta, momento, definition_mode, selected_vars, selected_blocks,
...
def build_total_results_from_global_summary(fecha_desde_lote=None, fecha_hasta_lote=None):
    ...
    resumen_cols = [
        ...
        "FECHA_DESDE", "FECHA_HASTA", "ORDEN_REGISTRO", "N_REGISTROS_INFORME",
    ...
    if fecha_hasta_lote is not None:
        add_paragraph(doc, f"Periodo analizado hasta: {fecha_hasta_lote.strftime('%Y-%m-%d')}")
    else:
        add_paragraph(doc, "Fecha final: último archivo disponible de cada participante")
...
def make_graph(df, var, out_png, momento, short_window=7, long_window=28):
    """Genera una gráfica con los valores válidos disponibles.

    Para HR, cuando el OCR no devuelve el valor directo pero existe RR medio,
    se calcula HR = 60000 / RR_MEAN. No se generan gráficos vacíos.
    """
    short_window, long_window = safe_window_pair(short_window, long_window)
    work = df.copy()
    ...
    values = pd.to_numeric(work[var], errors="coerce")

    if var == "HR" and "RR_MEAN" in work.columns:
        rr = pd.to_numeric(work["RR_MEAN"], errors="coerce")
        derived_hr = 60000.0 / rr.where(rr > 0)
        values = values.fillna(derived_hr)

    if var == "LnRMSSD" and "RMSSD" in work.columns:
        rmssd = pd.to_numeric(work["RMSSD"], errors="coerce")
        derived_ln = np.log(rmssd.where(rmssd > 0))
        values = values.fillna(derived_ln)

    sub = pd.DataFrame({
        "ORDEN_DIA": pd.to_numeric(work.get("ORDEN_DIA"), errors="coerce"),
        "FECHA_IMAGEN": work.get("FECHA_IMAGEN"),
        var: values,
    })
    sub = sub.dropna(subset=[var]).reset_index(drop=True)
    ...
    if str(momento) == "1":
        ...
        orden = pd.to_numeric(sub["ORDEN_DIA"], errors="coerce")
        x = orden.fillna(pd.Series(np.arange(1, n + 1), index=orden.index)).to_numpy(dtype=float)
        x_labels = None
    ...
    ma_visual = moving_average(pd.Series(y), 3, min_periods=1)
    ax.plot(
        x, ma_visual, color="black", linewidth=1.8, marker="o", markersize=5.5,
    ...
    if n >= 2:
        ...
        if tr is not None:
            ...
            ax.text(
                0.99, 0.03, f"y = {slope:.4f}x + {intercept:.4f}",
    ...
    first_vals = y[:min(3, n)]
    last_vals = y[-min(3, n):]
    comparison = compute_window_comparison(pd.Series(y), short_window=short_window, long_window=long_window)
    return {
        "n_valid": n,
        "slope": slope,
        "intercept": intercept,
...
def add_intro_block_with_optional_photo(doc, tipo_informe, nombre_sujeto, orden_registro, fecha_desde, fecha_hasta, fecha_informe, foto_participante_path=None, short_window=None, long_window=None, **kwargs):
    ...
    if fecha_hasta:
        textos.append(f"Periodo analizado hasta: {fecha_hasta}")
...
def convert_word_to_pdf(docx_path):
    """Convierte el Word a PDF usando Microsoft Word o LibreOffice."""
    docx_path = Path(docx_path).resolve()
    pdf_path = docx_path.with_suffix(".pdf")

    # 1) Microsoft Word en Windows, sin requerir pywin32.
    if os.name == "nt":
        ps_script = (
            "$ErrorActionPreference='Stop'; "
            "$word=New-Object -ComObject Word.Application; "
            "$word.Visible=$false; "
            f"$doc=$word.Documents.Open('{str(docx_path).replace("'", "''")}'); "
            f"$doc.SaveAs([ref]'{str(pdf_path).replace("'", "''")}', [ref]17); "
            "$doc.Close(); $word.Quit();"
        )
        try:
    ...
    for candidate in candidates:
        ...
        try:
            completed = subprocess.run(
                [str(candidate), "--headless", "--convert-to", "pdf", "--outdir", str(docx_path.parent), str(docx_path)],
                capture_output=True, text=True, timeout=120,
            )
            if completed.returncode == 0 and pdf_path.exists() and pdf_path.stat().st_size > 0:
                return pdf_path
...
def generate_word(df, summary_df, nombre_sujeto, orden_registro, graph_info, selected_vars, fecha_informe, output_docx, tipo_informe, fecha_desde=None, fecha_hasta=None, definition_mode="directa", logo_path=None, foto_participante_path=None, short_window=7, long_window=28, selected_blocks=None, swc_factor=0.5):
    doc = Document()
    set_margins(doc)
    ...
    def render_variable_block(doc, var, heading_prefix):
        ...
        if "grafico" in selected_blocks:
            ...
            if png and Path(png).exists() and Path(png).stat().st_size > 0:
                picture_paragraph = doc.add_paragraph()
                picture_paragraph.alignment = WD_ALIGN_PARAGRAPH.CENTER
                picture_run = picture_paragraph.add_run()
                picture_run.add_picture(str(Path(png).resolve()), width=Inches(6.2 if es_participante else 6.3))
...
def pedir_fecha_desde():
    while True:
        entrada = input("Fecha inicial (dd/mm/aaaa o aaaa-mm-dd): ").strip()
...
def pedir_fecha_desde_lote():
    ...
    print("El periodo incluye tanto la fecha inicial como la fecha final.")
    print("Se buscarán datos desde la fecha inicial en todos los participantes seleccionados.")
    ...
    while True:
        entrada = input("Fecha final (dd/mm/aaaa o aaaa-mm-dd) [Enter = último archivo de cada participante]: ").strip()
        if not entrada:
            return fecha_desde, None
        fecha_hasta = parse_fecha_usuario(entrada)
        if fecha_hasta is None:
            print("Fecha no válida. Ejemplos correctos: 31/07/2026 o 2026-07-31")
            continue
...
def obtener_ultima_fecha_archivos(files):
    fechas = []
    for f in files:
        fecha_txt, _ = extract_datetime_from_filename(f.name)
        fecha_img = pd.to_datetime(fecha_txt, errors="coerce") if fecha_txt else pd.NaT
        if not pd.isna(fecha_img):
            fechas.append(fecha_img.normalize())
    return max(fechas) if fechas else None


def filtrar_imagenes_por_periodo(files, fecha_desde, fecha_hasta=None):
    seleccionadas = []
    descartadas_sin_fecha = []
    descartadas_fuera_periodo = []
    for f in files:
        ...
        if pd.isna(fecha_img):
            ...
            continue
        fecha_normalizada = fecha_img.normalize()
        if fecha_normalizada >= fecha_desde.normalize() and (fecha_hasta is None or fecha_normalizada <= fecha_hasta.normalize()):
            seleccionadas.append(f)
        else:
            descartadas_fuera_periodo.append(f)
    return seleccionadas, descartadas_sin_fecha, descartadas_fuera_periodo


def filtrar_dataframe_para_informe(df, fecha_desde, fecha_hasta, orden_dia):
    if df is None or df.empty:
        return df.copy()
    out = df.copy()
    ...
    if fecha_hasta is not None:
        fechas = pd.to_datetime(out["FECHA_IMAGEN"], errors="coerce")
        out = out.loc[fechas <= fecha_hasta.normalize()].copy()
...
def collect_execution_config(tipo_informe_clave, default_name=None, images_dir=None, global_save_mode=None, fecha_desde=None, fecha_hasta=None):
...
def ejecutar_informe(tipo_informe_clave, tipo_informe_label, default_name=None, preconfig=None):
    ...
    fecha_hasta_solicitada = config.get("fecha_hasta")
    ...
    fecha_hasta = fecha_hasta_solicitada or obtener_ultima_fecha_archivos(files)
    if fecha_hasta is None:
        print("No se pudo determinar la fecha del último archivo de este participante.")
        return False
    files_en_rango, files_sin_fecha, files_fuera_rango = filtrar_imagenes_por_periodo(files, fecha_desde, fecha_hasta)
    print(f"Periodo aplicado: {fecha_desde.strftime('%Y-%m-%d')} a {fecha_hasta.strftime('%Y-%m-%d')}")
    print(f"Imágenes dentro del periodo: {len(files_en_rango)}")
    if files_sin_fecha:
        print(f"Imágenes omitidas por no poder extraer fecha del nombre: {len(files_sin_fecha)}")
    if files_fuera_rango:
        print(f"Imágenes omitidas por estar fuera del periodo indicado: {len(files_fuera_rango)}")
    ...
    required_metadata_cols = [
        "RUN_ID", "FECHA_EJECUCION", "NOMBRE_SUJETO", "TIPO_INFORME", "FECHA_DESDE", "FECHA_HASTA",
    ...
    df_informe = filtrar_dataframe_para_informe(df_final, fecha_desde, fecha_hasta, momento)
    nuevas_informe = 0
    if rows:
    ...
    if df_informe is None or df_informe.empty:
        print("")
        print(f"Sin registros válidos entre {fecha_desde.strftime('%Y-%m-%d')} y {fecha_hasta.strftime('%Y-%m-%d')} para el orden {momento}.")
    ...
    saved_pdf = convert_word_to_pdf(saved_docx)
    if saved_pdf:
        print(f"PDF generado: {saved_pdf}")
    else:
        print("AVISO: no se pudo generar el PDF. Instala Microsoft Word o LibreOffice para activar la conversión automática.")
    ...
    print(f"Registros incluidos en el informe (periodo y orden seleccionados): {len(df_informe)}")
...
def ejecutar_analisis():
    ...
    shared_configs = {}
    fecha_desde_lote, fecha_hasta_lote = pedir_fecha_desde_lote()
    ...
    if fecha_hasta_lote is None:
        print("Fecha final: último archivo disponible de cada participante")
    else:
        print(f"Fecha final común: {fecha_hasta_lote.strftime('%Y-%m-%d')}")
    ...
    if total > 1:
        ...
        print(f"Participantes omitidos por falta de datos en el periodo seleccionado: {participantes_sin_datos}")
    ...
    total_xlsx, total_docx = build_total_results_from_global_summary(fecha_desde_lote=fecha_desde_lote, fecha_hasta_lote=fecha_hasta_lote)
...
def main():
    if sync_public_data is None:
        print("\nERROR: no se puede crear Datos_publicos porque falta el módulo modules/public_data.py.")
        if PUBLIC_DATA_IMPORT_ERROR:
            print(f"Detalle técnico: {PUBLIC_DATA_IMPORT_ERROR}")
    else:
        try:
            public_summary = sync_public_data(SCRIPT_DIR)
            print(
                f"\nDatos_publicos creada/actualizada: {public_summary['participants']} participantes codificados; "
                f"{public_summary['images']} imágenes nuevas o actualizadas."
            )
            if public_summary.get("renamed"):
                print(f"Códigos conservados tras cambios de nombre de carpeta: {public_summary['renamed']}.")
        except Exception as exc:
            print(f"\nERROR: no se pudo crear o actualizar Datos_publicos: {exc}")
    ...
    while True:
        ...
        print("8. Preparar distribución oficial")
        print("9. Actualizar Datos_publicos en GitHub")
        print("10. Estado de los datos públicos")
        print("0. Salir")

        opcion = input("\nSelecciona una opción: ").strip()

        if opcion == "1":
            if sync_public_data is not None:
                try:
                    public_summary = sync_public_data(SCRIPT_DIR)
                    print(
                        f"\nDatos_publicos comprobada: {public_summary['participants']} participantes codificados; "
                        f"{public_summary['images']} imágenes nuevas o actualizadas."
                    )
                    if public_summary.get("renamed"):
                        print(f"Códigos conservados tras cambios de nombre de carpeta: {public_summary['renamed']}.")
                except Exception as exc:
                    print(f"\nERROR: no se pudo actualizar Datos_publicos: {exc}")
            ejecutar_analisis()
            break
        ...
        if opcion == "8":
            if mostrar_preparar_distribucion is None:
                print("El módulo de distribución no está disponible.")
                if DISTRIBUTION_IMPORT_ERROR:
                    print(f"Detalle técnico: {DISTRIBUTION_IMPORT_ERROR}")
                print("\nComprueba que exista el archivo: modules/distribution.py")
                input("Pulsa ENTER para volver al menú...")
            else:
                mostrar_preparar_distribucion(PROJECT_VERSION, SCRIPT_DIR)
            continue

        if opcion == "9":
            if publish_public_data_to_github is None:
                print("El módulo de datos públicos no está disponible.")
                if PUBLIC_DATA_IMPORT_ERROR:
                    print(f"Detalle técnico: {PUBLIC_DATA_IMPORT_ERROR}")
            else:
                print("\nSolo se preparará y publicará la carpeta Datos_publicos.")
                confirmacion = input("¿Deseas continuar? (s/n): ").strip().lower()
                if confirmacion in {"s", "si", "sí"}:
                    ok, mensaje = publish_public_data_to_github(SCRIPT_DIR)
        ...
        if opcion == "10":
            if show_public_data_status is None:
                print("El módulo de datos públicos no está disponible.")
            else:
                show_public_data_status(SCRIPT_DIR)
            input("\nPulsa ENTER para volver al menú...")
            continue
